...
const SHOPIFY_STORE_PERMANENT_DOMAIN = import.meta.env.VITE_SHOPIFY_STORE;
...
export async function createShopifyCart(item: Omit<CartItem, 'lineId'>): Promise<{ cartId: string; checkoutUrl: string; lineId: string } | null> {
...
export async function addLineToShopifyCart(cartId: string, item: Omit<CartItem, 'lineId'>): Promise<{ success: boolean; lineId?: string; cartNotFound?: boolean }> {
