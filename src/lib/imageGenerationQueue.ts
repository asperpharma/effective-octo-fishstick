...
type QueueEventCallback = (data: QueueItem | QueueStats | { message: string } | QueueItem[]) => void;
...
class ImageGenerationQueue {
  ...
  private emit(event: QueueEventType, data: QueueItem | QueueStats | { message: string } | QueueItem[]) {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
