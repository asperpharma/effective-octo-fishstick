...
type QueueEventCallback = (data: QueueItem | QueueStats | { error: string } | undefined) => void;
...
class ImageGenerationQueue {
  ...
  private emit(event: QueueEventType, data: QueueItem | QueueStats | { error: string } | undefined) {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
