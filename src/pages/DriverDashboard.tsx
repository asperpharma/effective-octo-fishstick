...
const statusConfig: Record<string, { label: string; labelAr: string; color: string; icon: React.ReactNode }> = {
  pending: { label: 'Pending', labelAr: 'قيد الانتظار', color: 'bg-yellow-500', icon: <Clock className="h-4 w-4" /> },
  confirmed: { label: 'Confirmed', labelAr: 'مؤكد', color: 'bg-blue-500', icon: <CheckCircle2 className="h-4 w-4" /> },
  processing: { label: 'Processing', labelAr: 'قيد التجهيز', color: 'bg-purple-500', icon: <Package className="h-4 w-4" /> },
  out_for_delivery: { label: 'Out for Delivery', labelAr: 'في الطريق', color: 'bg-orange-500', icon: <Truck className="h-4 w-4" /> },
  delivered: { label: 'Delivered', labelAr: 'تم التوصيل', color: 'bg-green-500', icon: <CheckCircle2 className="h-4 w-4" /> },
  cancelled: { label: 'Cancelled', labelAr: 'ملغي', color: 'bg-red-500', icon: <Clock className="h-4 w-4" /> },
};
      ...
      <div className="grid grid-cols-2 gap-3 p-4">
        ...
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-4 text-center">
            ...
            <p className="text-2xl font-bold text-green-600">{completedOrders.filter(o => o.status === 'delivered').length}</p>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-2 text-sm">
                    ...
                    <span className="text-muted-foreground">{order.delivery_address}, {order.city}</span>
                  </div>
                  <div className="flex items-center justify-between pt-2 border-t">
                    <span className="font-bold text-primary">{order.total.toFixed(2)} JOD</span>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center justify-between">
                  ...
                  <Badge className={`${statusConfig[selectedOrder.status]?.color || 'bg-gray-500'} text-white`}>
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-medium flex items-center gap-2">
                      <Package className="h-4 w-4" /> Items ({selectedOrder.items.length})
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      {selectedOrder.items.map((item, idx) => (
                        <div key={idx} className="flex justify-between text-sm py-1 border-b last:border-0">
                          ...
                          <span className="font-medium">{(item.price * item.quantity).toFixed(2)} JOD</span>
                        </div>
                      ))}
